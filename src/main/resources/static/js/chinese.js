const chineseLinkList = [];
const chineseNameList = [];

 async function getLinks() {
    let response = await fetch("http://localhost:8080/language/2").
    then((result) => result.json()).
    then((language) => language.videoDTOList);
    response.forEach(element => {
        chineseLinkList.push(element.link)
    });
    return response;
}
 async function fetchVideoNames() {
    ...
    then((language) => language.videoDTOList);
    response.forEach(element => {
        chineseNameList.push(element.name)
    });
    return response;
}
...
headerCell1.textContent = 'Chinese Link';
...
console.log("List :".concat(chineseLinkList));

// Loop through the arrays to create table rows
for (let i = 0; i < chineseLinkList.length; i++) {
...
button.src = chineseLinkList[i]; 

button.onclick = function() {
  window.open(chineseLinkList[i], "_blank");
};
...
row.appendChild(cell1);
  ...
  cell2.textContent = chineseNameList[i];
