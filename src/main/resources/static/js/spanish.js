const spanishLinkList = [];
const spanishNameList = [];

 async function getLinks() {
    let response = await fetch("http://localhost:8080/language/103").
    then((result) => result.json()).
    then((language) => language.videoDTOList);
    response.forEach(element => {
        spanishLinkList.push(element.link)
    });
    return response;
}
 async function fetchVideoNames() {
    ...
    then((language) => language.videoDTOList);
    response.forEach(element => {
        spanishNameList.push(element.name)
    });
    return response;
}
...
headerCell1.textContent = 'Spanish Link';
...
console.log("List :".concat(spanishLinkList));

// Loop through the arrays to create table rows
for (let i = 0; i < spanishLinkList.length; i++) {
...
button.src = spanishLinkList[i]; 

button.onclick = function() {
  window.open(spanishLinkList[i], "_blank");
};
...
row.appendChild(cell1);
  ...
  cell2.textContent = spanishNameList[i];
