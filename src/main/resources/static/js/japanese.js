const japaneseLinkList = [];
const japaneseNameList = [];

 async function getLinks() {
    let response = await fetch("http://localhost:8080/language/152").
    then((result) => result.json()).
    then((language) => language.videoDTOList);
    response.forEach(element => {
        japaneseLinkList.push(element.link)
    });
    return response;
}
 async function fetchVideoNames() {
    ...
    then((language) => language.videoDTOList);
    response.forEach(element => {
        japaneseNameList.push(element.name)
    });
    return response;
}
...
headerCell1.textContent = 'Japanese Link';
...
console.log("List :".concat(japaneseLinkList));

// Loop through the arrays to create table rows
for (let i = 0; i < japaneseLinkList.length; i++) {
...
button.src = japaneseLinkList[i]; 

button.onclick = function() {
  window.open(japaneseLinkList[i], "_blank");
};
...
row.appendChild(cell1);
  ...
  cell2.textContent = japaneseNameList[i];
