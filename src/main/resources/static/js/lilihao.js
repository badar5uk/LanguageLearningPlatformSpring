...
const videoNameList = [];
 ...
 async function getLinks() {
    let response = await fetch("http://localhost:8080/tutor/getTutorVideos?tutorId=52").
    then((result) => result.json()).
    then((tutor) => tutor.tutorVideo);
    response.forEach(element => {
        liliLinkList.push(element.link)
    });
    return response;
}
...
console.log("List :".concat(liliLinkList));

// Loop through the arrays to create table rows
for (let i = 0; i < liliLinkList.length; i++) {
...
button.src = liliLinkList[i]; 

button.onclick = function() {
  window.open(liliLinkList[i], "_blank");
};
