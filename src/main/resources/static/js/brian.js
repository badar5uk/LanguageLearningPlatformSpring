...
const videoNameList = [];
 ...
 async function getLinks() {
    let response = await fetch("http://localhost:8080/tutor/getTutorVideos?tutorId=4").
    then((result) => result.json()).
    then((tutor) => tutor.tutorVideo);
    response.forEach(element => {
        brianLinkList.push(element.link)
    });
    return response;
}
...
console.log("List :".concat(brianLinkList));

// Loop through the arrays to create table rows
for (let i = 0; i < brianLinkList.length; i++) {
...
button.src = brianLinkList[i]; 

button.onclick = function() {
  window.open(brianLinkList[i], "_blank");
};
